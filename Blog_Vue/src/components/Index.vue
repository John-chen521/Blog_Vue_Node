    ...
    <el-container id="index" class="animate__animated animate__fadeInUp">
      <el-row :gutter="12">
        <el-col :xs="24" :sm="17">
          ...
          <el-pagination :small="pagSmall" background @size-change="handleSizeChange"
            @current-change="handleCurrentChange" :page-size="queryInfo.pagesize" :current-page="queryInfo.pagenum"
            :layout="pagLayout" :total="totalcount">
          </el-pagination>
        </el-col>
        <el-col :xs="24" :sm="7">
          <el-card style="background-color: rgba(255,255,255,0.9)"
            ...
            <ul>
              <li class="animate__animated animate__fadeInUp blog-type" v-for="type in typeList" :key="type.id"
                @click="selectType(type.id)" :class="type.id === typeId? 'activeType':''">
